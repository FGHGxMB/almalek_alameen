...
class ReceiptFormScreen extends StatefulWidget {
  ...
  const ReceiptFormScreen(
      {Key? key, this.receiptToEdit, this.targetSuffix = ''})
      : super(key: key);
...
class _ReceiptFormScreenState extends State<ReceiptFormScreen> {
  ...
  void initState() {
    ...
    _suffixToUse = widget.receiptToEdit != null
        ? widget.targetSuffix
          ...
          return StreamBuilder<int>(
              ...
              builder: (context, snapshot) {
                ...
                return AlertDialog(
                  ...
                  content: const Text(
                      'هل أنت متأكد من حذف هذا السند نهائياً؟ سيتم عكس الأرصدة.'),
                  actions: [
                    TextButton(
                    ...
                    ElevatedButton(
                      onPressed: isReady
                          ? () {
                              cubit.deleteReceipt(widget.receiptToEdit!);
                              Navigator.pop(ctx);
                            }
                          : null,
                      style:
  ...
  void _showPrintShareDialog(BuildContext context, bool isShare, UserModel currentUser, List<CustomerModel> customers) async {
    ...
    try {
      final ownerDoc = await FirebaseFirestore.instance.collection('users').doc(widget.receiptToEdit!.delegateId).get(const GetOptions(source: Source.cache));
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              ...
              if (isShare) {
                ...
                try {
                  final bytes = await _screenshotController.captureFromWidget(widgetToCapture, delay: const Duration(milliseconds: 200), context: context, targetSize: const Size(384, 800), pixelRatio: 3.0);
                  final directory = await getApplicationDocumentsDirectory();
                  final imagePath = '${directory.path}/receipt_${widget.receiptToEdit?.delegateReceiptNumber.toString().padLeft(5, '0')}.png';
                ...
                try {
                  final bytes = await _screenshotController.captureFromWidget(widgetToCapture, delay: const Duration(milliseconds: 200), context: context, targetSize: const Size(384, 800), pixelRatio: 1.0);
  ...
  Widget _buildReceiptWidgetForImage(
      String cName, String cAddress, String cPhone, String dName) {
    return Container(
      width: 400,
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
              'سند قبض رقم: ${widget.receiptToEdit?.delegateReceiptNumber.toString().padLeft(5, '0') ?? ''}'),
          ...
          if (dName.isNotEmpty)
            Row(children: [Text('المندوب المستلم: $dName')]),
          const Divider(thickness: 2),
          ...
          Row(
            ...
            children: [
              ...
              Text('${_formatNum(widget.receiptToEdit!.amount)} ل.س',
          ...
          if (widget.receiptToEdit!.lineNote.isNotEmpty)
            Row(children: [
              Expanded(
                  child: Text('البيان: ${widget.receiptToEdit!.lineNote}',
  ...
  Widget build(BuildContext context) {
    ...
    final isMine = widget.receiptToEdit == null ||
        widget.receiptToEdit!.delegateId == currentUser.id;
    final canEdit = (isMine && currentUser.permissions.receiptEdit) ||
        (!isMine && currentUser.permissions.receiptEditMonitored);
    final canDelete = (isMine && currentUser.permissions.receiptDelete) ||
        (!isMine && currentUser.permissions.receiptDeleteMonitored);
        ...
        child: Scaffold(
          appBar: AppBar(
            ...
            title: Text(isViewMode
                ? 'عرض سند قبض #${widget.receiptToEdit!.delegateReceiptNumber.toString().padLeft(5, '0')}'
                : (widget.receiptToEdit != null
                    ? 'تعديل السند'
                    : 'إنشاء سند قبض')),
            centerTitle: true,
            backgroundColor:
                isViewMode ? Colors.grey.shade700 : Colors.green.shade600,
                      ...
                      if (state is ReceiptFormReady) {
                        return PopupMenuButton<String>(
                          onSelected: (val) {
                            ...
                            if (val == 'delete')
                              _showDeleteDialog(
                                  context, context.read<ReceiptFormCubit>());
                            if (val == 'print')
                              _showPrintShareDialog(
                                  context, false, currentUser, state.customers);
                            if (val == 'share')
                              _showPrintShareDialog(
                                  context, true, currentUser, state.customers);
                          },
                          itemBuilder: (ctx) => [
                            const PopupMenuItem(
                                value: 'print',
            ...
            builder: (context, state) {
              ...
              if (state is ReceiptFormReady) {
                ...
                if (isViewMode &&
                    ...
                    widget.receiptToEdit!.creditorAccount.isNotEmpty) {
                  try {
                    _selectedCustomer = state.customers.firstWhere(
                        (c) => c.id == widget.receiptToEdit!.creditorAccount);
                  } catch (e) {}
                }
                      ...
                      children: [
                        ...
                        Autocomplete<CustomerModel>(
                          ...
                          displayStringForOption: (c) =>
                              _cleanCustomerName(c.customerName, _suffixToUse),
                          optionsBuilder: (textEditingValue) {
                            if (isViewMode)
                              return const Iterable<
                                  CustomerModel>.empty(); // في العرض نعطل القائمة
                          ...
                          fieldViewBuilder: (context, controller, focusNode,
                            ...
                            return TextFormField(
                              ...
                              validator: (val) => _selectedCustomer == null
                        ...
                        TextFormField(
                          ...
                          validator: (val) {
                            ...
                            if (d <= 0)
                              return 'لا يمكن أن يكون المبلغ صفراً أو سالباً';
                            return null;
                          },
                        ),
                        const SizedBox(height: 16),
