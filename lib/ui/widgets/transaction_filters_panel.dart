...
class _TransactionFiltersPanelState extends State<TransactionFiltersPanel> {
  ...
  Future<void> _selectDate(BuildContext context, bool isFrom) async {
    ...
    if (picked != null) {
      setState(() {
        if (isFrom) _draft.fromDate = picked;
        else _draft.toDate = picked;
      });
    }
  }
                    ...
                    children:[
                      Expanded(
                        child: OutlinedButton.icon(
                          ...
                          label: Text(_draft.fromDate == null ? 'من تاريخ' : DateFormat('yyyy-MM-dd').format(_draft.fromDate!)),
                      ...
                      Expanded(
                        child: OutlinedButton.icon(
                          icon: const Icon(Icons.date_range, size: 16),
                          label: Text(_draft.toDate == null ? 'إلى تاريخ' : DateFormat('yyyy-MM-dd').format(_draft.toDate!)),
                ...
                ExpansionTile(
                  ...
                  children: widget.usersMap.keys.map((dId) {
                    ...
                    return CheckboxListTile(
                      ...
                      onChanged: (v) { setState(() { if (v == true) _draft.selectedDelegates.add(dId); else _draft.selectedDelegates.remove(dId); }); },
                    );
                  }).toList(),
                ),
